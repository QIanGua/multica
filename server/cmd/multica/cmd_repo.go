...
var repoCheckoutCmd = &cobra.Command{
	...
	Long: "Creates a git worktree from the daemon's bare clone cache. Used by agents to check out repos on demand. " +
		"This command talks to the local daemon over a version-coupled private protocol, so run it as `\"$MULTICA_CLI\" repo checkout <url>` " +
		"(`& $env:MULTICA_CLI repo checkout <url>` in PowerShell): " +
		"the daemon exports MULTICA_CLI with the absolute path of the binary it runs, while a bare `multica` resolves through PATH and may find an older install.",
	Args: exactArgs(1),
