...
Use this command only after migrations 352 and 353 are applied and every
...
to reduce load or run a bounded canary. Completion is explicit: do not depend on
complete historical activity ordering until the command logs `remaining=0`.

Migration 353 builds the serving index before this operator-run backfill so
