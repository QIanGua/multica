...
type UpdateAgentRuntimeRequest struct {
	...
	// can bind agents) and "public" (any workspace member can). Editing the
	// flag is owner / workspace admin only, gated by canEditRuntime.
...
// administrative decision. Admins keep canEditRuntime, so the way to open a
// runtime up is to flip it to `public` — an explicit, visible act rather than
// a silent bypass. This is the same rule the clients enforce
// (`isRuntimeUsableForUser` in packages/core/runtimes/access.ts), so UI, API
// and CLI agree. See migration 083 for the visibility column.
