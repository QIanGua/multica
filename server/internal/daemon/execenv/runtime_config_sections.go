...
func writeWorkflowIssue(b *strings.Builder, ctx TaskContextForEnv) {
	...
	b.WriteString("Status reflects the state the ISSUE is in, not your run's lifecycle — step 3 records the start; at the end of your work, write the state it reached. Write a status only when the new value differs from the current one, whoever the assignee is:\n\n")
