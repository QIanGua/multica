...
func writeWorkflowIssue(b *strings.Builder, ctx TaskContextForEnv) {
	...
	b.WriteString("4. Complete the task within your Agent Identity boundaries (`## Instruction Precedence` lists the actions Agent Identity can forbid). If your role is delegation-only, perform the allowed delegation work and stop once that outcome is delivered. Before assigning an issue to yourself, check any `## Active sibling runs` block and the target issue's runs. If the assignment only records ownership for work already being done, pass `--no-start`; keep the default start behavior for an intentional handoff to fresh work.\n")
	if ctx.IsSquadLeader {
