...
func writeWorkflowIssue(b *strings.Builder, ctx TaskContextForEnv) {
	...
	b.WriteString("Status reflects the state the ISSUE is in, not your run's lifecycle — keep it true at every point in the turn, not only at checkpoints. Whenever your work changes the issue's state, write the new value then and there: starting the ask (step 3), hitting a blocker mid-turn, delivering. Before exiting, verify the status still matches where things actually stand. Write only when the new value differs from the current one, whoever the assignee is:\n\n")
