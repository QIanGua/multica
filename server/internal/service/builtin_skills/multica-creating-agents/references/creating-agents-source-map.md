...
| Claude per-model effort resolution | `claude_catalog.go` `claudeEffortAllowForModel`, `thinking.go` `claudeModelEffortAllow` / `loadClaudeThinkingByModel` | Discovered `capabilities.effort` wins per model; the hand-maintained `claudeModelEffortAllow` answers for models the API did not describe. Both are then intersected with the `--effort` superset parsed from the local `claude --help`, because the installed CLI is what has to accept the flag. `effort.supported:false` yields an empty (not nil) allow-list, which hides the picker rather than inheriting the superset. The thinking cache key carries `claudeCatalogFingerprint` so a changed snapshot cannot be served a stale mapping |
