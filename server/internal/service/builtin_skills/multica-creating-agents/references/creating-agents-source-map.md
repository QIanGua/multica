...
| Workspace MCP layer resolved under the agent's | `internal/handler/workspace_mcp.go` `resolveWorkspaceMcpConfig`; applied in `daemon.go` `buildClaimedTaskResponse` | Union by server name with the agent winning; an agent config declaring zero servers opts out; read on every claim, so an admin edit lands on the agent's next task |
| Workspace MCP read/write API | `internal/handler/workspace_mcp.go` | `GET /api/workspaces/{id}/mcp-config` is member-visible but redacted for non-admins and agent actors (`server_count` stays visible); `PUT` is owner/admin, shape-validated, and full-replace |
