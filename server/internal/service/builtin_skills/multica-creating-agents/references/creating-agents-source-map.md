...
| Per-model effort probing | `acp_effort.go` `probeACPPerModelEffort`, wired via `acpDiscoveryProvider.probe` in `models.go` `discoverReasonixModels` | Inside the same discovery session, drives `session/set_config_option{configId:"model"}` once per remaining model and reads the effort option out of each refreshed response — the only way to learn a per-model catalog over ACP. The switch is confirmed against the runtime's own `currentValue` before the catalog is attributed, so a runtime that accepts the call without moving cannot have one model's catalog copied onto its siblings. Purely additive: a model that is unreachable, unconfirmed, or past `reasonixEffortProbeBudget` keeps `Thinking=nil` and shows no picker. Requires `isolatedStateEnv` — the sweep changes the session's model, and without isolation that would rewrite the model the user's own CLI opens on |
| Claude model catalog | `claude_catalog.go` `claudeAPICatalog` / `mergeClaudeModels`, `models.go` `claudeStaticModels` | `claude --help` cannot enumerate models, so the catalog is static by default. Where the daemon host's environment carries `ANTHROPIC_API_KEY` / `ANTHROPIC_AUTH_TOKEN`, `GET /v1/models` supplies the model list and `capabilities.effort.*.supported`; with no credential no request is made and behaviour is exactly the static table. The merge is a union, not a replacement: the API key's organisation and the CLI's own login are different auth contexts, so replacing could delete models the CLI can still run. Only the daemon ever calls this — the Multica server does not |
