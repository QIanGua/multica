...
| ACP effort catalogs are per model, not per session | `acp_effort.go` `annotateACPThinkingForSessionModel` | Annotates only the model marked `Default` (the advertised `currentModelId`), which is the one the handshake describes for free. ACP options may depend on each other, and reasonix v1.21.5 derives the catalog from the current model's provider entry — `deepseek-v4-flash` advertises `low`, `deepseek-v4-pro` does not, and some models expose no effort at all. Copying one catalog across models would offer levels the runtime then refuses; `probeACPPerModelEffort` reads the rest for real instead |
| Per-model effort probing | `acp_effort.go` `probeACPPerModelEffort`, wired via `acpDiscoveryProvider.probe` in `models.go` `discoverReasonixModels` | Inside the same discovery session, drives `session/set_config_option{configId:"model"}` once per remaining model and reads the effort option out of each refreshed response — the only way to learn a per-model catalog over ACP. The switch is confirmed against the runtime's own `currentValue` before the catalog is attributed, so a runtime that accepts the call without moving cannot have one model's catalog copied onto its siblings. Purely additive: a model that is unreachable, unconfirmed, or past the budget keeps `Thinking=nil` and shows no picker. Requires `isolatedStateEnv` — the sweep changes the session's model, and without isolation that would rewrite the model the user's own CLI opens on |
| Discovery must fit the client's poll window | `models.go` `modelDiscoveryClientPollTimeout` / `reasonixDiscoveryTimeout`, `acp_effort.go` `acpProbeCompletionReserve` | An uncached runtime is discovered on the request path — `InitiateListModels` only answers instantly when it already holds a snapshot — and `POLL_TIMEOUT_MS` in `packages/core/runtimes/models.ts` gives up at 30s. Any provider timeout at or past that makes the picker report a timeout while the daemon is still working. Every `acpDiscoveryProvider.timeout` must leave reporting headroom under it, and the probe sweep additionally stops `acpProbeCompletionReserve` before its context expires so the catalog it built survives. A slower binary covers fewer models rather than overrunning |
| Claude model catalog stays static | `models.go` `claudeStaticModels`, `claude_catalog.go` ("Why the model LIST stays static") | `claude --help` cannot enumerate models, and the Models API cannot safely supply them either: the catalog is discovered per RUNTIME, while tasks execute per AGENT with the agent's own `ANTHROPIC_API_KEY` layered in at launch (`layerCustomEnvAndHermesHome`). A daemon-env key for one organisation would publish its models into a picker used by an agent that executes with another's, which is "advertised but doesn't work". Adding a new Claude model therefore still needs a Multica release; making it dynamic needs an agent-scoped discovery request (protocol change, tracked separately) |
| Claude Models API discovery | `claude_catalog.go` `claudeAPICatalog` / `fetchClaudeAPIModels` | Credential-gated, effort-only, and narrowing-only. With `ANTHROPIC_API_KEY` / `ANTHROPIC_AUTH_TOKEN` in the daemon host's environment, `GET /v1/models` supplies `capabilities.effort.*.supported` — treat that answer as untrusted, because it is runtime-scoped while execution is agent-scoped; it may only shrink the shipped per-model table, never widen it. With no credential no request is made and behaviour is exactly the static table. Credentials are read only from the environment (never a config file or keychain); redirects are refused outright because net/http does not strip `x-api-key` across origins; concurrent misses are serialised so a failed fetch cannot overwrite a fresh success. Only the daemon ever calls this — the Multica server does not |
| Claude per-model effort resolution | `claude_catalog.go` `claudeEffortAllowForModel`, `thinking.go` `claudeModelEffortAllow` / `loadClaudeThinkingByModel` | Discovery may only NARROW the hand-maintained `claudeModelEffortAllow`, never widen it, and is ignored for a model with no baseline. The runtime-scoped credential that answers discovery is not the one a task runs with — an agent can set its own `ANTHROPIC_API_KEY` *and* `ANTHROPIC_BASE_URL` (documented router/proxy mode) — so a gateway answering `xhigh` for a model another gateway serves differently must not be able to publish that level. Intersecting makes the direction safe regardless of which context answered: the result is always a subset of what the build shipped. What it buys is fail-closed — a level Anthropic stops supporting disappears without a Multica release. The result is then intersected again with the `--effort` superset from the local `claude --help`. `effort.supported:false` narrows to an empty (not nil) allow-list, hiding the picker. The thinking cache key carries `claudeCatalogFingerprint` so a changed snapshot cannot be served a stale mapping |
