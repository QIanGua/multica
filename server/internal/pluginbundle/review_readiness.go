...
const (
	...
	releaseKeyID             = "multica-plugin-release-2026-01"
	// These values are generated by the offline release builder. The API ships
	// only the public trust root and the signature, never the signing key.
...
func reviewReadinessArchive() ([]byte, error) {
	...
	for _, file := range files {
		...
		if err != nil {
			return nil, fmt.Errorf("create bundled plugin entry: %w", err)
		}
		if _, err := entry.Write(file.content); err != nil {
			return nil, fmt.Errorf("write bundled plugin entry: %w", err)
		}
	}
	if err := writer.Close(); err != nil {
		return nil, fmt.Errorf("close bundled plugin archive: %w", err)
	}
	return buffer.Bytes(), nil
}

// ReviewReadinessRelease validates the embedded official release against the
// server's public trust root before it is allowed into the registry.
func ReviewReadinessRelease() (plugincontract.ValidatedRelease, []plugincontract.ArtifactFile, error) {
	archive, err := reviewReadinessArchive()
	if err != nil {
		return plugincontract.ValidatedRelease{}, nil, err
	}
	publicKey, err := base64.StdEncoding.DecodeString(releasePublicKeyBase64)
	if err != nil {
		return plugincontract.ValidatedRelease{}, nil, fmt.Errorf("decode bundled plugin trust root: %w", err)
	}
	signature, err := base64.StdEncoding.DecodeString(releaseSignatureBase64)
	if err != nil {
		return plugincontract.ValidatedRelease{}, nil, fmt.Errorf("decode bundled plugin signature: %w", err)
	}
	...
	release, err := plugincontract.ValidateReleaseCandidate(plugincontract.ReleaseCandidate{
		...
		SourceRef:      "bundled://software-delivery/1.0.0",
	...
	return release, artifact.Files, nil
}
