...
const (
	...
	// codexRequestUserInputDegradedAnswer is the structured tool result we send
	// back to Codex when this host cannot collect interactive answers. Empty
...
const (
	...
	multicaCodexRequestUserInputBeginMarker = "# BEGIN multica-managed request_user_input (do not edit; regenerated by daemon)"
	multicaCodexRequestUserInputEndMarker   = "# END multica-managed request_user_input"
)
...
func (b *codexBackend) executeOnce(ctx context.Context, prompt string, opts ExecOptions, attempt int) (*Session, error) {
	...
	if codexHome != "" {
		...
		if err := ensureCodexMcpConfig(configPath, opts.McpConfig, b.cfg.Logger); err != nil {
	...
	go func() {
		...
		c.serverAdvertisedInteractiveUserInput = codexInitializeAdvertisesInteractiveUserInput(initResult)
		b.cfg.Logger.Info("codex lifecycle", "phase", "initialize_response", "task_id", b.cfg.TaskID, "runtime_id", b.cfg.RuntimeID, "pid", cmd.Process.Pid, "attempt", attempt, "latency", time.Since(initializeStarted).Round(time.Millisecond).String(), "interactive_user_input", c.serverAdvertisedInteractiveUserInput)
...
func (c *codexClient) handleServerRequest(raw map[string]json.RawMessage) {
	...
	case codexMethodRequestUserInput:
		c.handleRequestUserInput(id, raw["params"])
	default:
...
func codexInitializeAdvertisesInteractiveUserInput(result json.RawMessage) bool {
	...
	var payload struct {
		...
		Capabilities     struct {
			RequestUserInput     bool `json:"requestUserInput"`
			InteractiveUserInput bool `json:"interactiveUserInput"`
		} `json:"capabilities"`
	}
...
type codexToolRequestUserInputQuestion struct {
	ID       string                            `json:"id"`
	Header   string                            `json:"header"`
	Question string                            `json:"question"`
	IsOther  bool                              `json:"isOther"`
	IsSecret bool                              `json:"isSecret"`
	Options  []codexToolRequestUserInputOption `json:"options"`
}
...
func ensureCodexRequestUserInputDisabled(configPath string, logger *slog.Logger) error {
	...
	block := multicaCodexRequestUserInputBeginMarker + "\n" +
		"[tools.experimental_request_user_input]\n" +
		"enabled = false\n" +
		multicaCodexRequestUserInputEndMarker + "\n"
	var updated string
	if stripped == "" {
		updated = block
	} else {
	...
	if logger != nil {
		logger.Debug("codex: pinned experimental_request_user_input disabled in config.toml",
