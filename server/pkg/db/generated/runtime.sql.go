...
func (q *Queries) FailTasksForOfflineRuntimes(ctx context.Context, reconnectGraceSecs float64) ([]AgentTaskQueue, error) {
	rows, err := q.db.Query(ctx, failTasksForOfflineRuntimes, reconnectGraceSecs)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
