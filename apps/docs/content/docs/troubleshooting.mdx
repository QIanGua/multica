...
An agent executes at most 6 runs concurrently by default; a single daemon executes at most 20. At the limit, new runs stay in the queue until an active one finishes. Runs also keep queueing while the runtime is offline; they only fail after going unclaimed for more than 2 hours by default (raise the limit with `MULTICA_TASK_QUEUED_TTL`, see [Environment variables](/environment-variables)).
