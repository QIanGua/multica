...
A custom property of type **Member or agent** is a reference, not an assignment. Naming an agent there records who is involved — a reviewer, an escalation contact — but creates no task and sends no notification. Only the assignee, or an @mention in a comment, starts a run.
