...
|---|---|
| `backlog` | Assigning the issue to an agent does not start a run. |
| `todo` | Moving the issue here starts the assigned agent. |
...
So a `Code Review` status in the `in_review` category ends an autopilot run exactly like `in_review` does, and a `Rework` status in the `todo` category starts the assigned agent exactly like `todo` does. The name is what your team reads; the category is what the platform acts on — wherever the rest of this documentation names a status, it is naming the category, and custom statuses inside it are covered too.
...
Built-in statuses are translated into the interface language; a custom status always shows the name you typed. The API and the CLI address it by its key, derived from the name when you create it: `multica issue status MUL-42 code_review`.
