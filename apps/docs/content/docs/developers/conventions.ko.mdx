...
사용자가 관리하는 지속적인 업무와 에이전트의 한 번 실행은 서로 다른 개념입니다.

| 엔티티 | en | zh-Hans | ja | ko |
| --- | --- | --- | --- | --- |
| 등록된 업무 단위(`issue`) | Issue | **任务** | **タスク** | **태스크** |
| 에이전트의 한 번 실행 기록(API / DB에서는 `task`) | **Run** | **运行** | **実行** | **실행** |

하나의 태스크에는 여러 실행이 연결될 수 있습니다. UI와 문서에서는 실행 기록을 `Task`나 '작업'으로 부르지 않고 **Run / 실행**을 사용합니다. `task`는 기존 API / DB 식별자로만 유지합니다.

- **API / DB 필드**는 `issue` / `task` / `skill`을 그대로 사용합니다: `issue_status`, `task_id`, `skill_uuid`.
- 프로토콜과의 대응을 보여 줘야 하는 개발자 문서에서는 'Run(API에서는 `task_id`)'으로 표기할 수 있습니다.
- 코드 참조와 리터럴 명령은 영어를 유지합니다: `multica issue ...`.
- `skill`은 정착된 번역어가 없는 Multica 고유 개념이므로 각 로케일의 기존 규칙을 따릅니다.
