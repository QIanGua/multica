...
ユーザーが管理する継続的な仕事と、エージェントによる 1 回の実行は別の概念です。

| エンティティ | en | zh-Hans | ja | ko |
| --- | --- | --- | --- | --- |
| 登録された仕事の単位（`issue`） | Issue | **任务** | **タスク** | **태스크** |
| エージェントの 1 回の実行記録（API / DB では `task`） | **Run** | **运行** | **実行** | **실행** |

1 つのタスクには複数の実行が紐づくことがあります。UI とドキュメントでは、実行記録を `Task` や「作業」と呼ばず、**Run / 実行**を使います。`task` は既存の API / DB 識別子としてのみ残します。

- **API / DB フィールド**は `issue` / `task` / `skill` のままです：`issue_status`、`task_id`、`skill_uuid`。
- プロトコルとの対応を示す必要がある開発者向け文章では「Run（API では `task_id`）」と書けます。
- コード参照とリテラルコマンドは英語のままです：`multica issue ...`。
- `skill` は定着した訳語のない Multica 固有概念なので、各ロケールの既存ルールに従います。
