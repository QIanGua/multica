...
API key と base URL の両方が空の場合、このレイヤーは無効になり、サーバーはどのモデルプロバイダーにも送信しません。チャットの内容を自社のデプロイ外に出せないポリシーでは、これがサポートされた構成です。セッションはクライアントが最初のメッセージから生成したタイトルをそのまま使い、フォローアップ質問のボタンは表示されず、他の機能への影響はありません。
