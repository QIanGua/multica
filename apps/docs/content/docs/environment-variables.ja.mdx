...
| `MULTICA_TASK_QUEUED_TTL` | `2h` | 実行がキュー内でクレームされずに置かれることのできる最長時間。超過すると `queued_expired` で失敗します。低並行度のランタイムが長時間の実行の後ろに正当に待機しうる場合は大きくしてください |
...
| `MULTICA_WORKSPACES_ROOT` | `~/multica_workspaces` | 実行作業ディレクトリのルート |
...
| `workspaces_root` | `~` 配下のプロファイル別パス | 実行作業ディレクトリのルート |
