...
<Callout type="info">
これは補助生成レイヤーのみに関する説明です。エージェントの実行は別のデータ経路です。エージェントがチャットに応答するとき、デーモンはそのエージェントの AI コーディングツールをツール自身の資格情報で実行し、`MULTICA_*` 変数はエージェントの環境から取り除かれ、渡されません。上記の変数を空にしてもこの経路には影響しないため、エージェントのランタイム設定側で制御してください。
</Callout>
