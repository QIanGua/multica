...
- Runs already queued wait for the runtime to recover, for up to 2 hours by default (adjustable via `MULTICA_TASK_QUEUED_TTL`, see [Environment variables](/environment-variables)).
- Active runs fail; eligible issue or chat runs can retry automatically.
