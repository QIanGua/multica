...
<Callout type="info">
This covers the assist layer only. Running an agent is a separate data path: when an agent answers a chat, your daemon executes that agent's AI coding tool using the tool's own credentials, and `MULTICA_*` variables are dropped from the agent's environment rather than passed to it. Emptying the variables above does not affect it — govern that path through the agent's runtime configuration.
</Callout>
