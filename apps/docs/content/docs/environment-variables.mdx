...
When both the API key and the base URL are empty, this layer is disabled and the server sends nothing to any model provider. That is the supported configuration when your policy does not allow chat content to leave your deployment: chat sessions keep the title the client derives from the first message, the follow-up question buttons do not appear, and everything else is unaffected.
