...
| `MULTICA_TASK_QUEUED_TTL` | `2h` | How long a run may sit in the queue unclaimed before failing with `queued_expired`; raise it when low-concurrency runtimes can legitimately wait behind long-running work |
