...
| `MULTICA_TASK_QUEUED_TTL` | `2h` | 运行在队列中无人认领的最长时长，超过后以 `queued_expired` 失败；低并发运行时可能合理地排在耗时较长的运行之后，此时应调大 |
...
| `MULTICA_WORKSPACES_ROOT` | `~/multica_workspaces` | 运行工作目录的根目录 |
...
| `workspaces_root` | `~` 下按 profile 区分的路径 | 运行工作目录的根目录 |
