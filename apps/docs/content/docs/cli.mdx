...
| | `mcp get/set` | Read or replace the MCP servers shared by every agent in the workspace | `set` is owner/admin and replaces the whole document; `--mcp-config-file` / `--mcp-config-stdin` keep tokens out of shell history; `--mcp-config null` clears it |
