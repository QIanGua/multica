...
Do not set `MULTICA_DAEMON_PORT` in the host shell, Compose service, or container entrypoint that starts a daemon. The host daemon derives its health port from `--profile` and injects this variable into agent tasks itself. If an older deployment carries the variable at startup, remove it before running `multica login`; versions 0.4.22 and 0.4.23 otherwise mistake that shell for a managed task and reject login.
