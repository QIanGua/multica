...
| | `export [agent-id...]` | Write agents' portable configuration to a JSON document | `--all` (every non-archived agent), `--file` (written 0600; stdout otherwise). `custom_env`, `mcp_config`, and `runtime_config` are never exported — the document records only that they existed |
| | `import` | Create or update agents from an exported document | `--file` / `--stdin`, `--runtime-id` (omit only when re-importing into the workspace the document came from), `--on-conflict` (`fail` / `overwrite` / `rename` / `skip`) matched on agent name, `--into <agent-id>`, `--dry-run` |
