...
| | `property list/set/unset <issue-id>` | Set custom property values | `set`: `--name`, `--value` (`actor` / `multi_actor` accept a member or agent name, email, or id — referencing an agent never starts a run) |
