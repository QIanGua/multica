...
イシューの**実行ログ**を開き、まず実行の現在のステータスと待機理由を確認します。
...
エージェントはデフォルトで最大 6 件、1 つのデーモンはデフォルトで最大 20 件の実行を同時に処理します。上限に達すると、新しい実行は進行中のものが終わるまでキューに残ります。ランタイムがオフラインの間も実行はキューに残り続け、2 時間（デフォルト）を超えて引き受けられなかった場合にのみ失敗します（上限は `MULTICA_TASK_QUEUED_TTL` で変更可能、[環境変数](/environment-variables)）。
