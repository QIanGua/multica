...
API key와 base URL이 모두 비어 있으면 이 레이어가 꺼지고 서버는 어떤 모델 제공자에게도 요청을 보내지 않습니다. 채팅 내용이 배포 환경 밖으로 나가면 안 되는 정책이라면 이것이 지원되는 구성입니다. 세션은 클라이언트가 첫 메시지에서 만든 제목을 그대로 사용하고, 후속 질문 버튼은 표시되지 않으며, 나머지 기능은 영향을 받지 않습니다.
