...
打开 issue 的**执行日志**，先看运行的当前状态和等待原因。
...
智能体默认最多同时执行 6 条运行；单个守护进程默认最多同时执行 20 条。达到上限时，新运行会留在队列中，等已有执行结束后再开始。运行时离线时，运行也会继续排队；超过 2 小时（默认值）仍未领取才会失败（上限可用 `MULTICA_TASK_QUEUED_TTL` 调整，见[环境变量](/environment-variables)）。
...
否则通常只需等前一条运行结束。若前一条已经卡住，可以从它的执行日志中停止；也可以为当前智能体选择其他本地目录。这个目录互斥由守护进程在内存中维护，不写磁盘锁文件；怀疑锁状态异常时运行 `multica daemon restart` 即可释放，没有需要手动删除的文件。
...
Usage 页面读取按小时汇总的数据，不直接读取每条运行的原始用量。先确认原始数据和汇总表：
